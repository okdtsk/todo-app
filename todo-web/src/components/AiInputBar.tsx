...
import { Send, ImagePlus, Link, Loader2, Sparkles, X, ChevronDown, ChevronUp } from "lucide-react";
...
export function AiInputBar({ onActionsDone }: AiInputBarProps) {
  ...
  return (
    <div className="mt-1">
      ...
      {hasMessages && chatExpanded && (
        <div className="max-h-[300px] overflow-y-auto px-1 py-3 flex flex-col gap-3">
          {messages.map((msg, i) => (
            <div
              key={i}
            ...
            >
              {msg.image && (
                <img
                  src={msg.image}
                  alt="Uploaded"
                  className="max-w-[200px] max-h-[120px] rounded-lg object-cover"
                />
              )}
              <div
      ...
      {hasMessages && (
        <div className="flex items-center justify-between px-1 py-1">
          <button
            ...
            className="flex items-center gap-1 text-[11px] text-text-tertiary hover:text-text-secondary transition-colors duration-150"
          >
            {chatExpanded ? <ChevronDown size={12} /> : <ChevronUp size={12} />}
            {chatExpanded ? "Hide" : `Show ${messages.length} messages`}
          </button>
          <button
            onClick={() => {
              setMessages([]);
        ...
        <div className="px-1 pb-2">
          <div className="relative inline-block">
            <img
              src={imagePreview}
              alt="Preview"
              className="max-w-[100px] max-h-[60px] rounded-lg object-cover"
            />
            <button
              onClick={clearImage}
              className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-text text-bg rounded-full flex items-center justify-center"
            >
              <X size={8} />
            </button>
          </div>
      ...
      <form onSubmit={handleSubmit} className="py-3">
        <div className="flex items-center gap-2">
          <Sparkles size={16} className="text-text-tertiary flex-shrink-0" />
          <input
            type="text"
            value={input}
            ...
            className="flex-1 text-[13px] text-text bg-transparent outline-none placeholder:text-text-tertiary"
            disabled={loading}
          />
